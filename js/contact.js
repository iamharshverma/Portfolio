...
$(document).ready(function() {
    ...
    $(document).on('click', '.contact-topic-pill', function(e) {
        ...
        if (topic) {
            ...
            if (window.AnalyticsService) {
                AnalyticsService.trackTopicSelect(topic);
            }
        }
    });
    ...
    $('#contact-form').on('submit', function(e) {
        ...
        if (!validation.isValid) {
            // GA4 Tracking: Validation failed
            if (window.AnalyticsService) {
                AnalyticsService.trackContactSubmission('validation_error', {
                    formId: 'contact-form',
                    errors: validation.errors,
                    errorCount: validation.errors.length,
                    invalidFields: validation.errors.map(function(err) { return err.field; })
                });
            }
        ...
        // GA4 Tracking: Submission Attempt
        if (window.AnalyticsService) {
            AnalyticsService.trackContactSubmission('attempt', {
                formId: 'contact-form',
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // GA4 Tracking: Conversion / Lead Success
                if (window.AnalyticsService) {
                    AnalyticsService.trackContactSubmission('success', {
                        formId: 'contact-form',
                        subject: formData.subject,
                        organization: formData.organization,
            ...
            error: function(xhr) {
                ...
                // GA4 Tracking: Submission Error
                if (window.AnalyticsService) {
                    AnalyticsService.trackContactSubmission('error', {
                        formId: 'contact-form',
